...
if __name__ == '__main__':
    ...
    label2id = {}
    id2label = {}
    for i in range(0,4): #classes of 0 to 3
    ...
    #subsample
    train_df = train_df.dropna(subset=['text'], ignore_index=True).drop_duplicates(subset=['text'], ignore_index=True).groupby(['generator', 'language']).apply(lambda x: x.sample(min(10000, len(x)), random_state = random_seed)).groupby(['label']).apply(lambda x: x.sample(min(10000, len(x)), random_state = random_seed)).sample(frac=1., random_state = random_seed).reset_index(drop=True)
    valid_df = valid_df.dropna(subset=['text'], ignore_index=True).drop_duplicates(subset=['text'], ignore_index=True).groupby(['generator', 'language']).apply(lambda x: x.sample(min(1000, len(x)), random_state = random_seed)).groupby(['label']).apply(lambda x: x.sample(min(500, len(x)), random_state = random_seed)).sample(frac=1., random_state = random_seed).reset_index(drop=True)
    
    # train detector model
    if args.test_only != True:
      fine_tune(train_df[['text', 'label']], valid_df[['text', 'label']], f"models_C/{model}", id2label, label2id, model, args.continue_train)

    # test detector model
    if 'ID' not in test_df.columns: test_df['ID'] = test_df.index.astype(int)
    if 'label' not in test_df.columns: test_df['label'] = 0
    best_or_last = "best"
    if args.last == True: best_or_last = "last"
    results, predictions, probs, class_prob = test(test_df[['text', 'label']], f"models_C/{model}/{best_or_last}/", id2label, label2id)
    ...
    if best_or_last == "last": prediction_path = prediction_path.replace('.csv', '-last.csv')
